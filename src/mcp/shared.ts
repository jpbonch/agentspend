...
function toTextResult(payload: JsonObject, isError = false) {
  return {
    isError,
...
export function toolSuccess(payload: JsonObject) {
  return toTextResult({ ok: true, ...payload });
}

export function toolError(message: string, extra?: JsonObject) {
  return toTextResult(
    {
      ok: false,
      error: message,
...
function setupRequiredError(configureUrl: string | null) {
...
export async function resolveApiKeyForTool(apiClient: FerriteApiClient): Promise<string> {
  try {
    ...
    if (error.message.includes("No API key found")) {
      throw new McpToolError("Setup is required before I can continue.", "CONFIGURE_REQUIRED");
    }
